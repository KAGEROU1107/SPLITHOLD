...
export interface PublicBillInfo {
  ...
  registration_open: boolean
  slot_count: number
  // Bank details intentionally omitted — exposed only via /pay/[token] after joining
}
...
export async function createBill(
  ...
  // join_token is generated by Supabase via column default: gen_random_uuid()
  // Verify this default exists in the schema if migrating to a new DB instance.
...
export async function getBillByJoinToken(joinToken: string): Promise<PublicBillInfo | null> {
  const { data: bill, error } = await supabaseAdmin
    .from('bills')
    .select('id, title, description, due_date, status, amount_per_pax, max_participants, registration_open')
...
export async function deleteBill(
  ...
  if (!isAdmin && bill.organizer_id !== callerId) {
    return { ok: false, reason: 'forbidden' }
  }
